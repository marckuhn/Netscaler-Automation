...
#region Script Settings
    #region Test Environment variables
        $RootFolder = "C:\batch\scripts\nitro"
        $SubnetIP = "192.168.168"
...
If ($ConfigFirstLogon)
{
    ...
    #region Step 2. SNIP (NS - nsip)
        Add-NSIPResource -NSSession $NSSession -IPAddress ($SubnetIP+ ".3") -SubnetMask "255.255.255.240" -Type SNIP -ErrorAction SilentlyContinue
        Write-Host "Subnet IP Addresses: " -ForegroundColor Yellow -NoNewline
        Write-Host (Get-NSIPResource -NSSession $NSSession | Select-Object ipaddress,type,state | Where-Object {$_.type -eq "SNIP"} )
    #endregion

    #region Step 3b. DNS Server IP Address (Domain Name Service - dnsnameserver)
        Add-NSDnsNameServer -NSSession $NSSession -DNSServerIPAddress ($SubnetIP + ".93") -ErrorAction SilentlyContinue
